...
# Generate Nginx config for Higress Manager UI
# Serves the AI Gateway Management skill's web interface
# Also proxies API requests to Higress Console
cat > /etc/nginx/conf.d/higress-manager.conf << 'NGINX'
# AI Gateway Management UI (served from skill directory)
server {
    listen 18900;
    root /opt/hiclaw/agent/skills/ai-gateway-management/web;
    index index.html;
    ...
    # Proxy API requests to Higress Console
    location /session/ {
        proxy_pass http://127.0.0.1:8001/session/;
        proxy_http_version 1.1;
