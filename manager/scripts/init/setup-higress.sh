...
if [ ! -f "${SETUP_MARKER}" ]; then
    ...
    higress_api POST /v1/service-sources "Registering Higress Manager UI service source" \
        '{"name":"higress-manager","type":"static","domain":"127.0.0.1:18900","port":18900,"properties":{},"authN":{"enabled":false}}'
    ...
    higress_api POST /v1/domains "Creating Higress Manager domain" \
        '{"name":"manager-local.hiclaw.io","enableHttps":"off"}'
    ...
    # 7. Higress Manager UI Route (web interface for AI Gateway management)
    higress_api POST /v1/routes "Creating Higress Manager UI route" \
        '{"name":"higress-manager","domains":["manager-local.hiclaw.io"],"path":{"matchType":"PRE","matchValue":"/"},"services":[{"name":"higress-manager.static","port":18900,"weight":100}]}'

    # 7a. Enable basic-auth on Higress Manager UI route
    higress_api PUT /v1/routes/higress-manager/plugin-instances/basic-auth "Enabling basic-auth on Higress Manager UI route" \
        '{"version":null,"scope":"ROUTE","target":"higress-manager","targets":{"ROUTE":"higress-manager"},"pluginName":"basic-auth","pluginVersion":null,"internal":false,"enabled":true,"rawConfigurations":"consumers:\n  - name: admin\n    credential: '"${HICLAW_ADMIN_USER:-admin}"':'"${HICLAW_ADMIN_PASSWORD}"'"}'
...
# ============================================================
# Wait for AI plugin activation with smart polling
# First config takes ~45s, subsequent changes ~10s
# Instead of fixed sleep, actively probe until ready
# ============================================================
wait_for_ai_gateway() {
    local max_attempts=60
    local attempt=0
    local wait_interval=2
    local test_url="${CONSOLE_URL}/v1/ai/routes"
    
    log "Waiting for AI Gateway plugin activation (smart polling)..."
    
    while [ $attempt -lt $max_attempts ]; do
        attempt=$((attempt + 1))
        
        # Try to list AI routes - this confirms the AI plugin is active
        local http_code
        ...
        if [ "${http_code}" = "200" ]; then
            log "AI Gateway plugin activated after $((attempt * wait_interval)) seconds"
            return 0
        fi
        
        if [ $((attempt % 10)) -eq 0 ]; then
            log "Still waiting... ($((attempt * wait_interval))s elapsed, HTTP ${http_code})"
        fi
        
        sleep ${wait_interval}
    done
    
    log "WARNING: AI Gateway may not be fully activated after $((max_attempts * wait_interval)) seconds"
    log "Proceeding anyway - some features may require a container restart"
    return 1
}
