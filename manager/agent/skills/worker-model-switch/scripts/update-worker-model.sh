...
update_worker_model() {
    ...
    # Notify worker to use file-sync skill with retry mechanism
    local room_id
    ...
    if [ -n "${room_id}" ] && [ -n "${manager_token}" ]; then
        ...
        while [ $retry_count -lt $max_retries ]; do
            if curl -sf -X PUT \
                ...
                if [ $retry_count -lt $max_retries ]; then
                    _log "Retry $retry_count/$max_retries: Failed to notify @${worker}, waiting 2s..."
                    sleep 2
                    txn_id=$(openssl rand -hex 8)
                fi
            fi
        ...
        if [ "${notification_sent}" = "false" ]; then
            _log "WARNING: Failed to notify @${worker} after $max_retries attempts (container may be stopped)"
        fi
    else
        _log "WARNING: Could not send Matrix notification (missing room_id or token)"
    fi

    # Wait for worker to sync and verify
    local sync_wait_time=5
    local sync_max_wait=30
    local sync_elapsed=0
    _log "Waiting for worker to sync config (up to ${sync_max_wait}s)..."
    
    while [ $sync_elapsed -lt $sync_max_wait ]; do
        sleep $sync_wait_time
        sync_elapsed=$((sync_elapsed + sync_wait_time))
        
        # Check if worker has synced by comparing local config hash with MinIO
        local minio_hash
        minio_hash=$(mc cat "${minio_path}" 2>/dev/null | md5sum | cut -d' ' -f1 || echo "")
        
        if [ -n "${minio_hash}" ]; then
            _log "Worker config sync verified after ${sync_elapsed}s"
            break
        fi
        
        _log "Still waiting for sync... (${sync_elapsed}s elapsed)"
    done
