...
# ── Auto-restart OpenClaw to apply changes immediately ────────────────────────
log "Triggering OpenClaw config reload..."
RELOAD_ENDPOINT="http://127.0.0.1:18799/api/reload"
RELOAD_HTTP=$(curl -s -o /dev/null -w '%{http_code}' -X POST "${RELOAD_ENDPOINT}" \
    -H "Authorization: Bearer ${GATEWAY_KEY}" \
    --connect-timeout 5 --max-time 10 2>/dev/null) || RELOAD_HTTP="000"

if [ "${RELOAD_HTTP}" = "200" ] || [ "${RELOAD_HTTP}" = "204" ]; then
    log "OpenClaw config reload triggered successfully (HTTP ${RELOAD_HTTP})"
    log "Waiting for config to take effect..."
    sleep 1
    # Verify the model is now active
    CURRENT_MODEL=$(jq -r '.agents.defaults.model.primary' "${CONFIG_FILE}" 2>/dev/null)
    if [ "${CURRENT_MODEL}" = "hiclaw-gateway/${MODEL_NAME}" ]; then
        log "SUCCESS: Model switch complete. Active model: ${MODEL_NAME}"
        echo "MODEL_SWITCH_COMPLETE: ${MODEL_NAME}"
    else
        log "WARNING: Config file shows ${CURRENT_MODEL}, expected hiclaw-gateway/${MODEL_NAME}"
    fi
else
    log "WARNING: Could not trigger OpenClaw reload via API (HTTP ${RELOAD_HTTP})"
    log "Manual restart required: openclaw gateway restart"
fi
